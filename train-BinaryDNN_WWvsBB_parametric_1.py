...
class PerMassMetricsCallback(Callback):
    def __init__(self, model_reference, val_dataset, masses, output_dir):
        """
        ...
        :param val_dataset: Validation dataset (tf.data.Dataset).
    ...
    def filter_dataset_by_mass(self, dataset, mass):
        """Filter dataset by a specific mass value."""
        return dataset.filter(lambda features, label: tf.equal(features['mass'], mass))

    def on_epoch_end(self, epoch, logs=None):
        print(f"\nEvaluating per mass metrics after epoch {epoch + 1}...")
        for mass in self.masses:
            mass_dataset = self.filter_dataset_by_mass(self.val_dataset, mass)
            
            # Convert filtered dataset to a NumPy array for evaluation
            X_mass, Y_mass = [], []
            for features, label in mass_dataset:
                X_mass.append(tf.concat(list(features.values()), axis=-1).numpy())
                Y_mass.append(label.numpy())
            
            if len(X_mass) == 0:
                continue  # Skip if no data for this mass
            
            X_mass = tf.convert_to_tensor(X_mass)
            Y_mass = tf.convert_to_tensor(Y_mass)
            
            # Compute loss and accuracy for this mass
            metrics = self.model_reference.evaluate(X_mass, Y_mass, verbose=0)
...
def load_data(inputPath, variables, num_events, csv_path, metadata_path, signal_masses, feature_columns, label):
    """
    ...
    if csv_exists and metadata_exists:
        # Load CSV file efficiently using `make_csv_dataset`
        dataset = tf.data.experimental.make_csv_dataset(
            csv_path,
            batch_size=BATCH_SIZE,
            select_columns = feature_columns+[label],
            label_name=label,  # Set your label column name
            num_epochs=NUM_EPOCHS,
            shuffle=True
        )
        def cast_features(features, label):
            for key in features:
                features[key] = tf.cast(features[key], tf.float32)
            return features, tf.cast(label, tf.int32)

        return dataset.map(cast_features)  # Apply transformation inline
    ...
    for key in keys:
        if key == 'ggh':
            ...
            fileNames = ["VBF_HToZZTo2L2Nu_M1000" if mass==1000 else f"VBF_HToZZTo2L2Nu_M{mass}_TuneCP5_13TeV_powheg2_JHUGenV7011_pythia8" for mass in signal_masses]
            target = 1  # VBF
        else:  # Background
            fileNames = [
    "drellyan_HTbinned-100to200",
    "drellyan_HTbinned-200to400",
    "drellyan_HTbinned-400to600",
    "drellyan_HTbinned-600toInf",
    "drellyan_HTbinned-70to100",
    "drellyan_inclusive_herwig",
    "drellyan_inclusive",
    "drellyan_ptz_0to50",
    "drellyan_ptz_100to250",
    "drellyan_ptz_250to400",
    "drellyan_ptz_400to650",
    "drellyan_ptz_50to100",
    "drellyan_ptz_650toInf",
    "ST_s-channel_4f_hadronicDecays",
    "ST_s-channel_4f_leptonicDecays",
    ...
    "ZZZ"
        ...
        for filen in fileNames:
            ...
            class_data[key].append(chunk_df)
            data = pd.concat([data, chunk_df], ignore_index=True)

            print(f"Loaded {len(chunk_df)} events for process {key} with mass {mass}")

    # Second pass: Determine the class with the least number of events and adjust
    #class_lengths = {key: len(pd.concat(class_data[key])) for key in class_data}

    # Find the minimum number of events across all classes
    #min_events = min(class_lengths.values())

    # Adjust each class to have the same number of events (equal to the class with the least number of events)
    #adjusted_data = []
    #for key in class_data:
    #    # Concatenate all data from the class
    #    class_df = pd.concat(class_data[key])

    #    # If the class has more events than the minimum, randomly sample the class to match the minimum
    #    if len(class_df) > min_events:
    #        class_df = class_df.sample(n=min_events, random_state=42)
    #
    #    # Add the adjusted data to the final list
    #    adjusted_data.append(class_df)

    # Concatenate all adjusted class data into a single DataFrame
    #data = pd.concat(adjusted_data, ignore_index=True)
...
def train_model_with_mass_metrics_1(model, train_dataset, val_dataset, batch_size, epochs, output_dir, class_weight=None,label):
    """
    Train the model with accuracy and loss tracking for each mass point.
    ...
    :return: Training history and per mass metrics callback.
    """
    masses = [200, 300, 400, 500, 600, 700, 800, 900, 1000, 1500, 2000, 3000]
...
# Preprocessing function (Modify as per dataset)

#def preprocess(features,label):
#    features = tf.concat([tf.cast(features[col], tf.float32) for col in features], axis=-1)
#    label = tf.cast(label, tf.float32)
#    return features, label

# Convert all features to float32 directly

def size_of_dataset(csv_path, feature_list, label):
    # Load dataset (one epoch to get correct count)
    full_dataset = tf.data.experimental.make_csv_dataset(
        csv_path,
        batch_size=1,  # Set batch size to 1 for accurate counting
        select_columns=feature_list+[label],
        label_name=label,  # Replace with actual target column
        num_epochs=1,  # Load only once to avoid repetition
        shuffle=False  # No shuffle to ensure correct counting
    )
    #total_size = tf.data.experimental.cardinality(full_dataset.unbatch()).numpy()
    total_size = sum(1 for _ in full_dataset)
    print(f"total size: {total_size}")
    #for features, label in full_dataset.take(1):  # Get first batch
    #    print("Features:", features)
    #    print("Label:", label)
    # Function to filter class 0
    ggh_dataset = full_dataset.filter(lambda features, label: tf.squeeze(label) == 0)
    vbf_dataset = full_dataset.filter(lambda features, label: tf.squeeze(label) == 1)
    bkg_dataset = full_dataset.filter(lambda features, label: tf.squeeze(label) == 2)

    # Get counts using TensorFlow's cardinality function
    #nggh = tf.data.experimental.cardinality(ggh_dataset.unbatch()).numpy()
    #nvbf = tf.data.experimental.cardinality(vbf_dataset.unbatch()).numpy()
    #nbkg = tf.data.experimental.cardinality(bkg_dataset.unbatch()).numpy()
    
    nggh = sum(1 for _ in ggh_dataset)
    nvbf = sum(1 for _ in vbf_dataset) 
    nbkg = sum(1 for _ in bkg_dataset)
    return total_size, nggh, nvbf, nbkg
...
def main():
    ...
    feature_columns = feature_columns + ['mass']
    label = 'target'
    
    # Load data
    signal_masses = [200, 300, 400, 500, 600, 700, 800, 900, 1000, 1500, 2000, 3000]
    full_dataset = load_data(args.inputPath, variables, args.num_events, csv_path=csv_path, metadata_path=metadata_path, signal_masses=signal_masses, feature_columns=feature_columns, label=label)
    ...
    n, nggh, nvbf, nbckg = size_of_dataset(csv_path, feature_columns, label)
    
    ## commented by Sumit
    #n = len(data)
    #nggh = len(data.iloc[data.target.values == 0])
    #nvbf = len(data.iloc[data.target.values == 1])
    #nbckg = len(data.iloc[data.target.values == 2])
    #print("Total (train+validation) length of ggh = %i, vbf =%i, bckg = %i" % (nggh, nvbf, nbckg))
    #ggh, vbf, bckg = np.bincount(data['target'])
    ggh, vbf, bckg = nggh, nvbf, nbckg
    total = ggh + vbf + bckg
    print('Raw events:\n    Total: {}\n    ggh: {} ({:.2f}% of total)\n   vbf: {} ({:.2f}% of total)\n  Background: {} ({:.2f}% of total)\n'.format(total, ggh, 100 * ggh / total, vbf, 100 * vbf / total, bckg, 100 * bckg / total))
    # Scaling by total/2 helps keep the loss to a similar magnitude.
    # The sum of the weights of all examples stays the same.
    weight_for_0 = (1 / ggh)*(total)/3.0
    weight_for_1 = (1 / vbf )*(total)/3.0
    weight_for_2 = (1 / bckg)*(total)/3.0
    class_weight = {0: weight_for_0, 1: weight_for_1, 2: weight_for_2}
    print('Weight for class ggh: {:.2f}'.format(weight_for_0))
    print('Weight for class vbf: {:.2f}'.format(weight_for_1))
    print('Weight for class bckg: {:.2f}'.format(weight_for_2))
    print('#---------------------------------------')
    ...
    train_dataset = full_dataset.take(train_size)  # First `train_size` batches for training
    val_dataset = full_dataset.skip(train_size)   # Remaining for validation

    #def func(i):
    #    i = i.numpy() # decoding from the EagerTensor object
    #    x, y = preprocess(train_dataset[i])
    #    return x, y # numpy arrays of types uint8, float32

    #z = list(range(len(train_dataset))) # the index generator

    #train_dataset = tf.data.Dataset.from_generator(lambda: z, tf.uint8)

    #train_dataset = dataset.map(lambda i: tf.py_function(func=func, inp=[i], Tout=[tf.uint8, tf.float32]), num_parallel_calls=12)
    
    #z1 = list(range(len(val_dataset))) # the index generator

    #val_dataset = tf.data.Dataset.from_generator(lambda: z1, tf.uint8)

    #val_dataset = dataset.map(lambda i: tf.py_function(func=func, inp=[i], Tout=[tf.uint8, tf.float32]), num_parallel_calls=12)

    # Step 5: Apply preprocessing, batching, and prefetching
    train_dataset = (
        train_dataset
        .map(preprocess)
        .cache()
        .shuffle(1000)  # Shuffle only training data
        .repeat(NUM_EPOCHS)
    ...
    if os.path.exists(model_path):
        ...
        # Build model
        model = build_model(input_dim=len(feature_columns), learn_rate=args.learn_rate)
        ...
        history, per_mass_callback = train_model_with_mass_metrics_1(
    ...
    # Initialize lists to store features (x_val) and labels (y_val)
    features_list = []
    labels_list = []
    ...
    # Convert lists of features and labels to Pandas DataFrames
    # Assuming features is a dictionary of feature names and values
    X_val = pd.DataFrame({key: [tf.numpy_function(lambda x: x.numpy(), [value], tf.float32).numpy() for value in features_list] for key, value in features_list[0].items()})
    Y_val = pd.DataFrame(labels_list)

...
#SBATCH --output=/hpcfs/bes/mlgpu/sharma/ML_GPU/HHWWyy/logSlurm_Hype_%u_%x_%j.out
...
dirTag="April1_HyperParsScan_v2"
time(python train-BinaryDNN_HyperParScan.py -t 1 -s ${dirTag} -p 1  -m Nadam)
# time(python -m json.tool HHWWyyDNN_binary_${dirTag}_%j_BalanceYields/model_serialised.json > HHWWyyDNN_binary_${dirTag}_BalanceYields/model_serialised_nice.json)
date
##########################################
# Work load end

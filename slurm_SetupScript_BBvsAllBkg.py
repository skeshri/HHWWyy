...
# LogDirPath = "/hpcfs/bes/mlgpu/sharma/ML_GPU/HHWWyy/HHBByyvsAllDNN_binary_"+dirTag+"_BalanceYields/"
LogDirPath = MacroPath + "/HHBByyvsAllDNN_binary_"+dirTag+"_"+args.weights+"/"

print ("args.scan: ",args.scan)
if args.scan:
  CommandToRun = "python train-BinaryDNN_BBvsAllBkg.py -t "+ str(args.isTrain) + " -ModelToUse "+ str(args.ModelToUse) +" -s "+dirTag+" -p 1 -g 0 -r 1"  # Scan using RandomizedSearchCV
  # CommandToRun = "python train-BinaryDNN_BBvsAllBkg.py -t "+ str(args.isTrain) + " -ModelToUse "+ str(args.ModelToUse) +" -s "+dirTag+" -p 1 -g 1 -r 0"  # Scan using RandomizedSearchCV
elif args.dynamic_lr:
  CommandToRun = "python train-BinaryDNN_BBvsAllBkg.py -i /hpcfs/bes/mlgpu/sharma/ML_GPU/Samples/DNN_MoreVar_v5_BScoreBugFix/ -t "+ str(args.isTrain) + " -ModelToUse "+ str(args.ModelToUse) +" -s "+dirTag+" -w "+args.weights +" -lr "+str(args.learnRate)+" -e "+str(args.epochs)+" -b "+str(args.batch_size)+" -o "+args.optimizer + " -j "+args.json + " -nHiddenLayer "+str(args.nHiddenLayer) + " -dropoutLayer "+str(args.dropoutLayer) + " -dlr "+str(args.dynamic_lr)
...
def CopyImportFiles(dir):
    ...
    os.system("cp train-BinaryDNN_BBvsAllBkg.py "+dir)
...
echo ""
echo "==================================================="
time(python -m json.tool HHBByyvsAllDNN_binary_%s_%s/model_serialised.json > HHBByyvsAllDNN_binary_%s_%s/model_serialised_nice.json)
echo ""
echo "==================================================="
echo ""
#echo "Convert the model.h5 to model.pb"
#time(python convert_hdf5_2_pb.py --input HHBByyvsAllDNN_binary_%s_%s/model.h5 --output HHBByyvsAllDNN_binary_%s_%s/model.pb)
#echo "==================================================="
echo "Time stamp:"
date
##########################################
